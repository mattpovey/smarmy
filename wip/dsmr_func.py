...
def sm_fetch(type, log_file):
    # Fetch a given set of fields from smartmeter and append to 'file'
    # type is 'e' for electricity and will use sme_fields
    # type is 'g' for gas and will use smg_fields
    ...
    if type == 'e':
        sm_ts = str(getattr(telegram, "P1_MESSAGE_TIMESTAMP").value)
        os.system('clear')
        print(" ------------------------- \n Electricity Readings \n ------------------------- \n")
        for field in sme_fields:
            sm_value = str(getattr(telegram, field).value) 
            sm_unit = str(getattr(telegram, field).unit)
            with open(log_file, 'a') as outfile: 
                outline = field + ", " + sm_value + ", " + sm_unit + ", " + sm_ts
                print(outline)
